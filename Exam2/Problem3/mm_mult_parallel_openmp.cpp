...
int main(int argc, char *argv[])
{
  ...
  {
    // Exam 2 Edit
    // OpenMP splitting up the for loop, need to make sure that the loop row and column
    // are dependent on the index (i) since that is how the threads are split

    // Get the column to work on
    int col = start_column + i;
    if (col % (dim_n) == 0)
    {
      col = 0;
    }
    ...
    //#pragma omp parallel for num_threads(thread_count) schedule(static, 1) shared(row, col)
