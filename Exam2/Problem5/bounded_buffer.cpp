...
/* Compilation on the UAH ECE Jetson Cluster (blackhawk.ece.uah.edu)
  ...
  following two module load commands
   ...
   to compile/link the program with routines in the util.o file type
      ...
      g++ bounded_buffer.cpp -o bounded_buffer_omp util.o -fopenmp 
...
void producer(int id) {
   ...
   for (int i=0;i<group_sz(id,num_items,num_producers);i++) { 
      ...
      cout << "empty val = " << value << endl;
      sem_getvalue(&full, &value);
      cout << "full val  = " << value << endl;
      */
...
int main (int argc, char *argv[]) {
   ...
   switch (test)
   {
     ...
     // the buffer is full.  Block the consumer if there is nothing
